...
import { ShieldCheck, Wallet, Zap, Headset, Star, ArrowRight, ChevronDown, Sparkles, BadgeCheck } from "lucide-react";
import api from "../api/client";
import CarCard from "../components/CarCard";
import Spinner from "../components/ui/Spinner";

const KEUNGGULAN = [
  { icon: ShieldCheck, title: "Armada Lengkap", desc: "Puluhan unit mobil dari city car hingga SUV dan luxury MPV, siap untuk berbagai kebutuhan.", accent: false },
...
export default function Home() {
  ...
  useEffect(() => {
    api
      ...
      .then(({ data }) => setMobils(data.slice(0, 4)))
    ...
    <div>
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:items-center lg:px-8 lg:py-28">
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-accent-400/30 bg-accent-500/15 px-4 py-1.5 text-sm font-medium text-accent-200">
              <Sparkles size={14} />
              Rental Mobil Terpercaya #1
            </span>
            <h1 className="mt-5 text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl">
              Sewa Mobil Mudah, <span className="text-blue-300">Perjalanan Nyaman</span>
            </h1>
            <p className="mt-5 max-w-xl text-lg text-blue-100">
              Ajukan permintaan booking mobil rental secara online tanpa perlu membuat akun. Armada lengkap
              dan proses cepat untuk kebutuhan perjalanan Anda.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                to="/katalog"
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-accent-600 px-7 py-3.5 text-base font-semibold text-white shadow-lg shadow-accent-900/30 transition-all duration-200 hover:-translate-y-0.5 hover:bg-accent-500 hover:shadow-xl"
              >
                Booking Sekarang
                <ArrowRight size={18} />
              </Link>
              <Link
                to="/tentang-kami"
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/30 bg-white/5 px-7 py-3.5 text-base font-semibold text-white backdrop-blur-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/10"
              >
                Tentang Kami
              </Link>
            </div>
          ...
          <div className="relative hidden lg:block">
            <div className="aspect-[4/3] w-full overflow-hidden rounded-2xl border border-white/10 shadow-2xl">
              <img
                src="https://picsum.photos/seed/hero-rental/900/700"
      ...
      {faq.length > 0 && (
        <section className="bg-slate-50 py-16">
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <h2 className="text-center text-3xl font-bold text-slate-900">Pertanyaan Seputar Rental</h2>
            ...
            <div className="mt-8 space-y-3">
              {faq.map((item, idx) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[var(--shadow-soft)] transition-shadow duration-200 hover:shadow-[var(--shadow-soft-lg)]"
                  >
                    <button
                      type="button"
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-700 to-blue-900">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-14 text-center sm:px-6 lg:px-8">
          ...
          <Link
            to="/katalog"
            className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-accent-600 px-7 py-3.5 text-base font-semibold text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:bg-accent-500 hover:shadow-xl"
          >
            Booking Sekarang
            <ArrowRight size={18} />
          </Link>
