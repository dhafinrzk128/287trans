...
export default function FloatingWhatsApp() {
  ...
  return (
    // On a phone, tapping this hands off to the WhatsApp app and the chat just
    // opens. On desktop it goes to WhatsApp Web, which is a dead end for anyone
    // not already signed in there — so on pointer devices the button widens on
    // hover to reveal the number itself, giving those visitors something they
    // can act on (call it, or type it into WhatsApp on their phone) instead of
    ...
    <a
      ...
      aria-label={`Chat via WhatsApp ${profile.whatsapp}`}
      className="wa-float btn-glow-whatsapp group fixed bottom-6 right-6 z-40 flex h-14 items-center justify-center rounded-full bg-emerald-700 px-[0.875rem] text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:bg-emerald-800"
    >
      ...
      <span className="hidden max-w-0 overflow-hidden whitespace-nowrap text-sm font-semibold opacity-0 transition-all duration-200 md:block md:group-hover:ml-2.5 md:group-hover:max-w-[12rem] md:group-hover:opacity-100">
        {profile.whatsapp}
      </span>
    </a>
  );
